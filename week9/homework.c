...
void main(void) {
    int acc_x, acc_y, acc_z,angle, mode;
    ...
    acc_x = ADCW - 340;                      
    //Seg4_out(acc_x);  // uncomment this line when finding digital offset value
    ...
    while((ADCSRA & 0x10) == 0); // ADIF=1
    acc_z = ADCW - 240;  
    //Seg4_out(acc_z); // uncomment this line when finding digital offset value
    ...
    if (angle >= 45) PORTC = 0x00; // turn on led
    else PORTC = 0xFF; // turn off led      
